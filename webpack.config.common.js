...
const ignoreComs = fse.readJsonSync(resolve('./ignorecom.json'));

const igFiles = [];

_.keys(ignoreComs).forEach(item => {
  ignoreComs[item].forEach(each => {
    igFiles.push(`*components/${item}/${each}.js`);
  });
});
...
export default {
  ...
  module: {
    rules: [
      { test: igFiles, loader: 'ignore-loader' },
      {
        test: /\.jsx?$/,
        use: {
          loader: 'babel-loader',
  ...
  plugins: [
    // new webpack.IgnorePlugin(/Spin/),
    // new webpack.IgnorePlugin(/Button/),
